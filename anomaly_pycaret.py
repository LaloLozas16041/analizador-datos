...
def anomalyPycaret():
    ...
    st.subheader("AutoML - Detección de Anomalías")
    if 'begin_ad' not in st.session_state:
        st.session_state.begin_ad = False

    if not st.session_state.begin_ad:
        st.info("Para comenzar, haga doble clic en el botón Comenzar")
        ...
        st.info("Para reiniciar AutoML, haga clic en el botón Restablecer")
        ...
        if len(dataset) > 0:
            ...
            st.write("### Selección del Modelo y Configuración")
            with st.form("model_form"):
                features = st.multiselect("Selecciona las características a incluir:", dataset.columns)
